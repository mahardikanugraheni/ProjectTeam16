...
def bmi_index(bmi):
    myjsonfile=open('makananolahraga.json','r')
    jsondata=myjsonfile.read()
    obj=json.loads(jsondata)
    # obj = json.dumps(jsondata)
    underweight = json.dumps(obj['Underweight'])
    normal = json.dumps(obj['Normal'])
    overweight = json.dumps(obj['OverWeight'])
    obesity = json.dumps(obj['Obesity'])

    if bmi < 18.5:
        messagebox.showinfo(f'BMI = {bmi} is Underweight', (underweight[:-1]))
    elif (bmi>18.5) and (bmi<24.9):
        messagebox.showinfo(f'BMI = {bmi} is Normal',(normal[:-1]))
    elif (bmi>24.9) and (bmi <29.9):
        messagebox.showinfo(f'BMI ={bmi} is Overweight',(overweight[:-1]))
    else:
        messagebox.showinfo(f'BMI = {bmi} is Obesity',(obesity[:-1]))
...
def open_bmi():
    ...
    next_btn= Button(frame3,text='Next',command=open_bmr)
...
def open_bmr():
    global input_berat,input_tinggi,input_umur,input_gender,hasil_bmr
    root = Tk()
    ...
    hasil_bmr.pack(ipadx=200,ipady=300)
    root.mainloop()
