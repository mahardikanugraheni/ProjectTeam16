...
def awal(window,firstpage,bgloginpage,bgsignuppage,awalkalkupage,bmibmr):
    ...
    button1 = tk.Button(a,text="Sign Up", bg='#fabb17',fg='white',border=0,font=('Arial',18),command=lambda:halaman_Signup(frame1,window,bgsignuppage,bgloginpage,awalkalkupage,bmibmr))
    button1.place(x=495,y=450,relwidth=0.1)
    button2 = tk.Button(a,text="Log In", bg='#fabb17',fg='white',border=0,font=('Arial',18),command=lambda:halaman_SignIn(frame1,window,bgloginpage,awalkalkupage,bmibmr))
    ...
    with open('database.csv','r')as file:
        ...
        for row in csv_reader:
            if row[1]==username and row[2]==password:
                ...
                # next_button = tk.Button(bglogin,text='Next', command=lambda:halaman_awal(frame),bg='#fabb17',fg='white',border=0,font=('Arial',14))
...
def halaman_SignIn(frame,window,bgloginpage,awalkalkupage,bmibmr):
    ...
    login_button = tk.Button(bglogin,text="Login",command=lambda:signin(username_entry,password_entry,frame2,window,awalkalkupage,bmibmr),bg='#fabb17',fg='white',border=0,font=('Arial',14))
...
def halaman_Signup(frame,window,converted1,converted2,converted3,converted4):
    ...
    signup_button = tk.Button(bgsignup,text="SignUp",command=lambda:signup(name_entry,user1_entry,password1_entry,frame3,window,converted2,converted3,converted4),bg='#fabb17',fg='white',border=0,font=('Arial',14))
...
def halaman_awal(frame, window, awalkalkupage, bmibmr):
    ...
    button1 = tk.Button(bghal_awal,text='BMI Calculator',command=lambda:open_bmi(frame4,window,bmibmr,awalkalkupage),bg='#fabb17',fg='white',border=0,font=('Helvetica',14))
    button1.place(x=450,y=350)
    button2 = tk.Button(bghal_awal,text='BMR Calculator',command=lambda:open_bmr(frame4,window,bmibmr,awalkalkupage),bg='#fabb17',fg='white',border=0,font=('Arial',14))
...
def open_bmi(frame,window,bmibmr,awalkalkupage):
    ...
    back_btn=Button(bgkalku,text='Back',bg="#fabb17",border=1,fg='white',font=('Arial',14),command=lambda:halaman_awal(frame5,window,awalkalkupage,bmibmr))
...
def open_bmr(frame,window,bmibmr,awalkalkupage):
    ...
    tombol_hitung = Button(bgkalku2, text="Hitung BMR", command=lambda:hitung_bmr(input_berat,input_tinggi,input_umur,input_gender),fg='white',bg='#fabb17',font=('Arial',14))
    tombol_hitung.place(x=690,y=380)    
    reset_btn=Button(bgkalku2,text='Reset',command=lambda:reset_entry(input_berat,input_tinggi,input_umur,input_gender),bg="#fabb17",border=1,fg='white',font=('Arial',14))
    reset_btn.place(x=495,y=380)
    back_btn=Button(bgkalku2,text='Back',bg="#fabb17",border=1,fg='white',font=('Arial',14),command=lambda:halaman_awal(frame6,window,awalkalkupage,bmibmr))
